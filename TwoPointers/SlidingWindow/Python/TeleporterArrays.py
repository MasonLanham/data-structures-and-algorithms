...
For each step, you can either move forward a step on an array, or move to a square in the other array where the number is the same as the number in your current square ("teleportation"). 
...
def maximum_score(arr1: list[int], arr2: list[int]) -> int:
    modulus = 10**9 + 7
    ptr1, ptr2 = 0, 0
    sum1, sum2, score = 0, 0, 0
    while(ptr1 < len(arr1) and ptr2 < len(arr2)):
        if(arr1[ptr1] < arr2[ptr2]):
            sum1 = (arr1[ptr1] + sum1) % modulus
            ptr1 += 1
        elif(arr2[ptr2] < arr1[ptr1]):
            sum2 = (arr2[ptr2] + sum2) % modulus
            ptr2 += 1
        else:
            score = (max(sum1, sum2) + arr1[ptr1] + score) % modulus
            sum1, sum2 = 0, 0
            ptr1 += 1
            ptr2 += 1
    while(ptr1 < len(arr1)):
    ...
    while(ptr2 < len(arr2)):
        sum2 = (arr2[ptr2] + sum2) % modulus
        ptr2 += 1
    score = (max(sum1, sum2) + score) % modulus
    return score
